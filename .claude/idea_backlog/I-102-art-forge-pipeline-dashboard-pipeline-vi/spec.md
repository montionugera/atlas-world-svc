...
### Resolved by prior-art survey (2026-08-25)

- OQ1 → per-brief ledger file; attempts stored as **compact one-line entries** (git-friendly appends; graduate to `.jsonl` past ~100 rows).
- OQ2 → staleness à la DVC/Dagster: each attempt stores a **sha256 of the normalized brief**; dashboard recomputes and flags cells whose hash differs — display-only, never auto-executes.
