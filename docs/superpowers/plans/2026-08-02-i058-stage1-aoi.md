...
**Correction — the code block above has two defects, both found during implementation and confirmed by review (2026-08-02). Implement the corrected form, not the literal listing.**

1. **Stale entities are never removed after a despawn.** The removal loop looks the ref up via `byId.get(id)?.ref`, but `byId` is rebuilt each pass from the live entity list — a despawned entity is *absent* from it, so the lookup yields `undefined`, the `if (ref)` guard skips, and `view.remove()` never fires. The entity stays in the client's view permanently. **Fix:** keep a `visibleRefs: Map<string, object>` on `ViewerSlot` alongside `visible`, populated when an entity is added, and remove using that map rather than `byId`. The brief's own despawn test ("removes an entity that disappears from the world entirely") fails against the listing as written — which is how this was caught.

2. **`ref: object` does not compile.** `StateView.add` / `.remove` accept `Ref = Schema | ArraySchema | MapSchema | CollectionSchema | SetSchema`, and `object` is not assignable to it. **Fix:** keep `ref: object` on the public `InterestEntity` interface (plain-object mocks in the tests depend on it) and add a narrow `as Ref` cast *only* at the three or four `StateView.add` / `.remove` call sites, with a one-line comment explaining why. Do **not** widen the field to `any` — that disables checking at every future call site and violates the no-unjustified-`any` constraint.
...
**Correction — this step also requires a change to `src/tests/f018-harness.ts`, which the task's file list omits. Found during implementation (2026-08-02).**

`updateInterest()` reaches for `this.room.interestManager`, but `buildTestRoom()` constructs a plain `TestRoom` object rather than a real `GameRoom`, so that property is `undefined`. Every harness-driven tick then throws `TypeError: Cannot read properties of undefined (reading 'update')` — and because `GameSimulationSystem.update()` wraps its whole ordered pass in one `try/catch`, the throw **silently aborts the rest of the tick**, including `processBattleMessages()`. The symptom is 12 failures across `f018-model-parity`, `f018-pack-spread`, and `f018-boss-spread`, all presenting as "damage never applied" — nowhere near the actual cause.

**Fix:** extend `buildTestRoom()` to construct an `InterestManager` and the two collectors, using the *same* options the room uses (`AOI_CONFIG.radius` / `hysteresis` / `cellSize`, and `candidateRadius = radius × hysteresis`). Add `interestManager` and the collectors to the `TestRoom` interface so the typing is honest. The harness's own doc comment already claims it mirrors `GameRoom.onCreate()` — this restores that promise.

**Do NOT** instead make `updateInterest()` defensive with `if (!this.room.interestManager) return`. That hides exactly this class of wiring bug and would let a mis-wired production room fail silently.

**Separately noted, out of scope for Stage 1:** the single `try/catch` around the whole ordered pass means any throw anywhere silently drops the remainder of the tick. That is a pre-existing hazard this incident newly evidenced, and it deserves its own fix.
