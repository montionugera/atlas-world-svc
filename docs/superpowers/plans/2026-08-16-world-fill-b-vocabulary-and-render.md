...
- Modify: `tools/mapforge/lib/basin-sheet.mjs:199-207` (fill lookup), `:647-665` (the legend block), and **the two stale provenance strings this task inherits from Plan A** — the `<desc>` (`:143`) and the footer line (`:729`) both letter `tools/mapforge/render-map.mjs` and `content/maps/cluster1-geography.json` into `cluster1-world.svg`, and Plan A Task 12 deleted both of those files. They are DRAWN BYTES, so Plan A could not correct them without reding G-RENDER-LOCK; this task is the one licensed to. Replace with the surviving truth: the sheet is drawn from `content/spine/` by `tools/mapforge/render-sheet.mjs`. **Nothing in the repo gates this** — no test names either string — so it is written into this file's Files list rather than left in a Plan A commit body. A `git grep -n 'render-map\.mjs\|cluster1-geography' -- game-client/assets/art/maps/` returning zero hits is the check.
