...
- **Prompt:** the brief paragraph taken **verbatim** from `docs/worldbuilding/A1-geography-cluster1.md`
  (newlines collapsed to spaces — byte-identical to how `atelier/art-forge/out/l1/briefs.tsv` stored it for the
...
- The T24/T48 arms call `buildBaseGraph()` from the shipped `atelier/art-forge/generate/charsheet.mjs`, so the
...
- 8 images + labelled sheet: `atelier/art-forge/out/bakeoff/` (git-ignored)
- Comparison sheet: `atelier/art-forge/out/bakeoff/_sheet.png` (2644×1016, grouped by subject, one row of four
...
`atelier/art-forge/generate/charsheet.mjs` already emits exactly this shape; only `steps`, `cfg` and the
