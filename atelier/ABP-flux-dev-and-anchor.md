...
- **Subjects and prompts:** identical to `ABP-flux-eval.md` — `A1-ART-05 Gildmark` and
  `A1-ART-04 Norhollow`, taken **byte-identical** from `atelier/art-forge/out/l1/briefs.tsv` by reading the
...
The creature path already has the governing law, recorded in `atelier/art-forge/prompts/style-laws.json`:
...
## What should change in `atelier/art-forge/`

Recommendations only — **no tooling under `atelier/art-forge/` was modified or committed** in this
evaluation. The generator and specs written during it live in the git-ignored
`atelier/art-forge/out/devtest/anchor/` so they are reproducible, and are listed here for promotion.

1. **Promote the block-in generator to `atelier/art-forge/generate/blockin.mjs`**, with specs at
   `atelier/art-forge/prompts/blockin/<subject>.json`. It is ~60 lines and shells out to ImageMagick; it has
   no npm dependency.
...
All git-ignored, under `atelier/art-forge/out/devtest/`:
...
```js
// atelier/art-forge/out/devtest/anchor/blockin.mjs  (proposed: generate/blockin.mjs)
