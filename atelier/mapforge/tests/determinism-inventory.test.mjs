...
// in both scans) and by a non-recursive top-level walk (`atelier/mapforge/cli/`
...
// Everything under atelier/mapforge/ EXCEPT lib/ (inventoried above) and tests/.
...
const INVENTORY = {
  ...
  // atelier/mapforge/gen-world.mjs). The canary sheet is unaffected by the
...
// atelier/mapforge/trig-helper.mjs — beside gen-world.mjs and render-sheet.mjs —
...
// atelier/mapforge/generate-world.mjs, and a helper dropped next to it is exactly
...
test("everything under atelier/mapforge/ outside lib/ carries no imprecise Math at all", () => {
  ...
  assert.deepEqual(actual, {},
    "a file under atelier/mapforge/ outside lib/ gained an imprecise Math call. lib/ is inventoried; this layer is not, " +
...
// atelier/mapforge/tests/generate-world.test.mjs' "REPRODUCIBLE" test, which
...
test("nothing on the committed-byte path reads a clock or a random number", () => {
  ...
  // atelier/mapforge/ except tests/. No name is written down here, so nothing can
  // be left off it.
...
//      spurious comment — 5 of the files under atelier/mapforge/ no longer
...
test("stripComments: an ESCAPED SLASH in a regex does not open a comment (hole 5)", () => {
  // The exact line the review found, from atelier/mapforge/tests/glyphs.test.mjs:
...
test("the scan's coverage is DERIVED from the tree: extension class and directory recursion", () => {
  // Every extension a file under atelier/mapforge/ could plausibly carry and be
