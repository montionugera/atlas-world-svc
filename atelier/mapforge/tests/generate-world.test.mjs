// atelier/mapforge/tests/generate-world.test.mjs — Task 10b: the whole CLI.
//
...
const CLI = join(ROOT, "atelier/mapforge/generate-world.mjs");
...
test("regions carry rings AND holes, and every region's levelBand is banded", { timeout: 240000 }, () => {
  ...
  // atelier/mapforge/tests/partition.test.mjs for the full reason.
...
test("SYNTHETIC_LOAD_BUDGET, PRE_WORLD_ATLAS_CHILDREN and PRE_WORLD_SEALANE_ID are gone", () => {
  ...
  // atelier/mapforge/generate-world.mjs` -> 0. The assertion below has always
