...
//
// node:zlib is used here and banned only in atelier/mapforge/lib/texture-bake.mjs,
...
test("the committed synthetic-density.svg is not stale", () => {
  ...
  assert.equal(
    ...
    svg,
    "run: node atelier/mapforge/render-sheet.mjs --sheet synthetic",
  );
});
...
/**
 * PNG decoding is `atelier/mapforge/lib/png-ink.mjs`'s job — this file carried
