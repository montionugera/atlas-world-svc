// atelier/mapforge/tests/fabric-sheet.test.mjs — Plan C Task 13's two review
...
test("the fabric sheet is drawn from the COMMITTED fabric, not from the spine", () => {
  const src = readFileSync(join(ROOT, "atelier/mapforge/lib/fabric-sheet.mjs"), "utf8");
...
test("the overlay sheet reads the baseline from the DRAFT folder, never from git", () => {
  const src = readFileSync(join(ROOT, "atelier/mapforge/lib/overlay-sheet.mjs"), "utf8");
...
test("the overlay sheet carries a per-continent area-delta table with BOTH sides", () => {
  ...
  // ones (atelier/asset-storybook/maps-index.json, art-manifest.json).
...
test("the sea:land ratio the REVIEW SURFACES publish is the one the gate measures", () => {
  // CITATION ROT, fifth occurrence in this repo. atelier/asset-storybook/maps-index.json
  ...
  for (const rel of ["atelier/asset-storybook/maps-index.json", "game-client/assets/art/art-manifest.json"]) {
...
test("the retired generator and its tests are gone", () => {
  for (const p of ["atelier/mapforge/gen-world.mjs", "atelier/mapforge/lib/world-gen.mjs",
                   "atelier/mapforge/tests/gen-world.test.mjs", "atelier/mapforge/tests/world-gen.test.mjs"])
...
test("the retired hardcodes appear in no CODE anywhere under tools/ or scripts/", () => {
  ...
  const files = [
    ...sourceFilesUnder(join(ROOT, "atelier/mapforge")).filter((f) => !f.startsWith("tests/"))
      .map((f) => join(ROOT, "atelier/mapforge", f)),
  ...
  assert.ok(codeOfFile(join(ROOT, "atelier/mapforge/generate-world.mjs")).includes("writeRun"),
  ...
  // delete this.
  assert.ok(readFileSync(join(ROOT, "atelier/mapforge/generate-world.mjs"), "utf8")
