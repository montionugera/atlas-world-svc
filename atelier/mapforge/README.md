...
content/spine/                                   the spine (source of truth)
        │
        ▼  node atelier/mapforge/render-sheet.mjs --sheet <cluster1|atlas>
...
node atelier/mapforge/render-sheet.mjs --sheet cluster1            # SVG only
node atelier/mapforge/render-sheet.mjs --sheet atlas               # SVG only
node atelier/mapforge/render-sheet.mjs --sheet cluster1 --png      # SVG + 512px review thumb (the committed one)
node atelier/mapforge/render-sheet.mjs --sheet cluster1 --check    # self-checks + drift check, write nothing
...
node atelier/mapforge/render-sheet.mjs --sheet atlas --png --png-width 2000
...
atelier/mapforge/render-sheet.mjs --sheet cluster1 --png` (or `--sheet atlas`)
