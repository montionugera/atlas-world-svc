#!/usr/bin/env node
// atelier/mapforge/fit-premises.mjs — the footprint fit, committed as a PROCEDURE.
//
...
//
// NOT ON THE COMMITTED-BYTE PATH, but it lives under atelier/mapforge/ and so is
// scanned by tests/determinism-inventory.test.mjs: no transcendental, no `**`,
// no clock, no random. Math.sqrt only.
//
// USAGE
//   node atelier/mapforge/fit-premises.mjs            # report the fit, write nothing
//   node atelier/mapforge/fit-premises.mjs --write    # rewrite content/world/premises/*.json
//
