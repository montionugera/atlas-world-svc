// atelier/mapforge/lib/continent-sheet.mjs — the continent zoom tier (spec §7.4).
//
...
function build({ repoRoot, continent, contentRoot, legendTierArg, glyphSizePx = null }) {
  ...
  for (const tp of world.terrainPatches) {
    ...
    if (!fill) {
      problems.push(
        `continent-sheet ${continent}: terrain patch "${tp.id}" has kind "${tp.terrainKind}" with no FILL_FOR entry (atelier/mapforge/lib/draft.mjs)`,
      );
      continue;
    }
    body.push(
  ...
  o.push(
    `<desc>Drawn by atelier/mapforge/render-sheet.mjs from content/world/resolved/continent-${nn}.json and content/world/fabric/continent-${nn}.json. Surveyed regions carry biome ink; reported regions carry the provenance hatch and no terrain claim.</desc>`,
  );
  o.push("<defs>");
