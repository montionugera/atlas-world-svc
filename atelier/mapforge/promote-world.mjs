#!/usr/bin/env node
// atelier/mapforge/promote-world.mjs — Plan C Task 12: idempotent promotion.
//
...
export function promoteWorld({ repoRoot = DEFAULT_REPO_ROOT, runDir, dryRun = false }) {
  ...
  for (const sheet of Object.keys(SHEETS).sort()) {
    const r = runTool({ repoRoot, script: "atelier/mapforge/render-sheet.mjs", args: ["--sheet", sheet, "--no-png"] });
