#!/usr/bin/env node
// atelier/mapforge/generate-world.mjs — Plan C: the generator CLI.
//
...
export function runPasses({ manifest, premises,
  ...
  // COMPUTE. cpuTimings is what atelier/mapforge/tests/generate-world.test.mjs'
  ...
  // RESERVED_STREAM_NAMES) and a source scan over atelier/mapforge reds on the
...
async function main() {
  ...
  const { SHEETS } = await import(pathToFileURL(join(REPO_ROOT, "atelier/mapforge/render-sheet.mjs")).href);
