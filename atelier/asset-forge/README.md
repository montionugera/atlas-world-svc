...
```bash
bash atelier/asset-forge/bake.sh <src.blend> <out.glb>
```
...
```bash
node atelier/asset-forge/validate.mjs <out.glb> --kind character
```

Also runs over an entire manifest (this is what CI runs):

```bash
node atelier/asset-forge/validate.mjs --manifest game-client/assets/manifest.json
```
...
```bash
node atelier/asset-forge/intake.mjs <out.glb> --key <manifest-key> --license "<license text>" [--dry-run]
```
...
# 2. Bake
bash atelier/asset-forge/bake.sh art-source/bespoke/<name>/source/<name>.blend /tmp/<name>.glb

# 3. Validate
node atelier/asset-forge/validate.mjs /tmp/<name>.glb --kind character

# 4. Add a row to art-source/LICENSES.md BEFORE intake

# 5. Intake (dry-run first if unsure)
node atelier/asset-forge/intake.mjs /tmp/<name>.glb --key mob:aggressive --license "CC0 (Kenney Mini Characters, kitbashed)" --dry-run
node atelier/asset-forge/intake.mjs /tmp/<name>.glb --key mob:aggressive --license "CC0 (Kenney Mini Characters, kitbashed)"
