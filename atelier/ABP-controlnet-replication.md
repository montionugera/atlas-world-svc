...
`atelier/art-forge/forge.config.json` and the four block-ins revised in commit `04e7a40`
...
- **Prompts:** the brief prose from `atelier/art-forge/briefs/*.json` plus the house style
...
- **Runner:** `env.mjs`'s own `generateEnv()` — the same code path as `node generate/env.mjs
  ...
  backgrounded.
- **Nothing on `atelier/art-forge/` was modified** by the generation run itself (the driver script
...
The 16 full-resolution PNGs live under `atelier/art-forge/out/`, which is **git-ignored by design** —
...
Contact sheet: `atelier/art-forge/out/env-replication/_sheet-replication.png` (4 rows × 4 columns —
...
- **The hires pass was not run** for any of the 16 cells — base pass only, matching what `env.mjs`
  currently builds.
- **`atelier/art-forge/artifact-gate.mjs` was not run** against these 16 outputs. The prior round
...
`atelier/art-forge/generate/contact-sheet.sh` and `atelier/art-forge/compare.sh` **do not work on this
...
`magick montage` instead: `atelier/art-forge/out/env-replication/_sheet-replication.png`, 4×4,
labelled by filename, git-ignored under `out/` like all other generated art.

## Artifacts

All git-ignored, under `atelier/art-forge/out/env-replication/`:
