...
**Date:** 2026-08-01 · **Branch:** `feat/F-024` · **Code:** `atelier/art-forge/artifact-gate.mjs` ·
**Tests:** `atelier/art-forge/tests/artifact-gate.test.mjs` ·
...
`atelier/art-forge/out/{flux,devtest,anchorcmp}` — **52 PNGs**, git-ignored, produced by the flux/dev/schnell
...
`atelier/art-forge/intake-art.mjs` runs the gate **inside its validate phase**, before any file copy or
...
node atelier/art-forge/artifact-gate.mjs <image.png> --corner-sheet /tmp/corners.png
node atelier/art-forge/artifact-gate.mjs <image.png> --json   # metrics for adjudication
# exit 0 = PASS, 1 = FLAG, 2 = usage/IO error

# intake refuses a flagged image
node atelier/art-forge/intake-art.mjs --src <png> --id art:... --group ... --title ... --note ...

# deliberate override — REQUIRES a written reason, which is recorded forever
node atelier/art-forge/intake-art.mjs ... --skip-artifact-gate "reviewed corner sheet, flat sky intentional"
```
...
```bash
cd atelier/art-forge && npm test          # 41 tests
```
...
- **Nothing under `content/`, no manifest, `art-groups.json` or `scripts/check_asset_manifest.mjs` was
  modified.** No images are committed; `atelier/art-forge/out/` is git-ignored.
...
(mont-pc) and copied back to `atelier/art-forge/out/` — the two `flux/` files renamed to the
