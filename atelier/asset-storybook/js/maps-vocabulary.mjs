 ...
 * That is not theoretical: atelier/mapforge/ is NOT copied into the storybook
 * container by default, so the imports below 404 unless the Dockerfile ships
 * lib/ (it now does — see atelier/asset-storybook/Dockerfile).
 */
 ...
 * to atelier/asset-storybook/index.html — where maps.mjs uses it, for img.src
 ...
 * `import(REPO_ROOT_REL + "atelier/mapforge/lib/draft.mjs")` asks for
 * `atelier/mapforge/lib/draft.mjs`, 404s, and the panel removes itself —
...
export async function mountVocabulary(section) {
  ...
  try {
    const [draft, glyphMod, lexRes] = await Promise.all([
      import(repoUrl("atelier/mapforge/lib/draft.mjs")),
      import(repoUrl("atelier/mapforge/lib/glyphs.mjs")),
