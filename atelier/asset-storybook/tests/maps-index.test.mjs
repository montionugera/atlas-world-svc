...
//
// maps-index.json (committed, sibling to this test's atelier/asset-storybook/)
// is the storybook's OWN registry of mapforge sheets. It must never drift
// from atelier/mapforge/render-sheet.mjs's SHEETS export — the source of truth
...
test("every mapforge SHEETS id appears in maps-index.json", () => {
  ...
  for (const id of Object.keys(SHEETS)) {
    assert.ok(
      indexedIds.has(id),
      `sheet "${id}" is registered in atelier/mapforge/render-sheet.mjs SHEETS ` +
        `but missing from atelier/asset-storybook/maps-index.json`,
    );
  }
});
  ...
  for (const sheet of index.sheets) {
    ...
    assert.ok(
      ...
      `${sheet.png} is ${bytes} bytes — a committed sheet PNG must be a ` +
        ...
        `with \`node atelier/mapforge/render-sheet.mjs --sheet ${sheet.id} --png\``,
    );
  }
});
  ...
  for (const sheet of loadIndex().sheets) {
    ...
    assert.equal(
      ...
      `${sheet.png} is ${pngWidth(abs)} px wide, not ${want} — a committed ` +
        ...
        `\`node atelier/mapforge/render-sheet.mjs --sheet ${sheet.id} --png\``,
    );
  }
});
...
//
// The measure decodes the committed bytes (atelier/mapforge/lib/png-ink.mjs) and
