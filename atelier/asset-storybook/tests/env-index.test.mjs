...
// art-forge environment renders under atelier/art-forge/out/env/.
//
...
// (atelier/art-forge/.gitignore ignores out/), so this gate is existence-based:
...
const ENV_DIR = join(REPO_ROOT, "atelier/art-forge/out/env");
const RUN_LOG_PATH = join(REPO_ROOT, "atelier/art-forge/runs/A1-ART-02.json");
...
// atelier/art-forge/. Re-renders of the same path overwrite: last event wins.
function runLogRendersByOut() {
  ...
  for (const line of readFileSync(RUN_LOG_PATH, "utf8").split("\n")) {
    ...
    byOut.set("atelier/art-forge/" + event.out, event);
  }
  return byOut;
}
...
test("env-index.json exists and is well-formed", () => {
  ...
  for (const row of rows) {
    ...
    assert.equal(
      row.file,
      `atelier/art-forge/out/env/${row.id}.png`,
...
test("every render under atelier/art-forge/out/env/ has a row — a render cannot hide", () => {
  ...
  assert.deepEqual(
    indexed,
    onDisk,
    "env-index.json and atelier/art-forge/out/env/ drift — a render exists " +
