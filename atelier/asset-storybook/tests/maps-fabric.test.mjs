// atelier/asset-storybook/tests/maps-fabric.test.mjs — the fabric census panel.
//
...
test("the roster comes from world.json's own continents[].fabric column", async () => {
  ...
  const loaded = await loadFabric({ fetchImpl, baseUrl: "http://x/atelier/asset-storybook/index.html" });
...
test("the panel is mounted by the Maps tab, not left as an unreferenced module", () => {
  const src = readFileSync(join(REPO_ROOT, "atelier/asset-storybook/js/maps.mjs"), "utf8");
...
test("the containerised storybook ships the fabric it reads", () => {
  ...
  const df = join(REPO_ROOT, "atelier/asset-storybook/Dockerfile");
  ...
  assert.match(text, /content\/world/,
    "atelier/asset-storybook/Dockerfile does not copy content/world/ — the fabric census panel would be " +
