...
//   container the document root is built by atelier/asset-storybook/Dockerfile,
//   which did not ship atelier/mapforge/ — so without a rule here the panel
...
test("repo paths resolve against the DOCUMENT, not this module's own depth", () => {
  ...
  // "../../atelier/mapforge/lib/draft.mjs" asked for tools/TOOLS/mapforge/...,
  // 404'd, and the panel removed itself while every test stayed green.
  const doc = "http://h:6007/atelier/asset-storybook/index.html";
  assert.equal(
    repoUrl("atelier/mapforge/lib/draft.mjs", doc),
    "http://h:6007/atelier/mapforge/lib/draft.mjs",
  );
  assert.equal(
  ...
  for (const modUrl of [
    "http://h:6007/atelier/asset-storybook/js/maps-vocabulary.mjs",
    "http://h:6007/atelier/asset-storybook/js/data/deeper.mjs",
  ]) {
    assert.notEqual(
      new URL("../../atelier/mapforge/lib/draft.mjs", modUrl).href,
      repoUrl("atelier/mapforge/lib/draft.mjs", doc),
  ...
  assert.equal(
    repoUrl(
      "atelier/mapforge/lib/glyphs.mjs",
      "http://c/atelier/asset-storybook/index.html",
    ),
    "http://c/atelier/mapforge/lib/glyphs.mjs",
  );
});
...
test("the modules the panel imports from the browser are dependency-free", () => {
  ...
  for (const rel of ["lib/draft.mjs", "lib/glyphs.mjs"]) {
    const src = readFileSync(join(REPO_ROOT, "atelier/mapforge", rel), "utf8");
...
test("the storybook image ships the drawing library the panel imports", () => {
  // The panel resolves atelier/mapforge/lib/* against the document root. The
  ...
  assert.match(
    dockerfile,
    /^COPY atelier\/mapforge\/lib atelier\/mapforge\/lib$/m,
    "atelier/asset-storybook/Dockerfile does not COPY atelier/mapforge/lib — the Maps vocabulary panel will 404 in the container",
  );
  assert.match(
    ignore,
    /^!atelier\/mapforge\/lib\/\*\**\*$/m,
    "atelier/asset-storybook/Dockerfile.dockerignore does not whitelist atelier/mapforge/lib — the COPY above has nothing to copy",
  );
