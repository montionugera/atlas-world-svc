...
export class ProjectileCollisionResolver {
  ...
  handleEntityCollision(projectile: Projectile, target: WorldLife): void {
    ...
    if (
      incomingCfg?.canBeDeflected &&
      this.deflectionResolver.checkDeflection(projectile, target)
    ) {
      return
    }

    const attacker =
      this.gameState.mobs.get(projectile.ownerId) || this.gameState.players.get(projectile.ownerId)
    ...
    if (this.battleManager) {
      ...
      if (attacker && attacker.isAlive) {
        ...
        try {
          ...
          console.warn(
            `⚠️ PROJECTILE: failed to emit BATTLE_DAMAGE_PRODUCED (hit): ${err instanceof Error ? err.message : String(err)}`
          )
        }
  ...
  handleProjectileCollision(projectileA: Projectile, projectileB: Projectile): void {
    ...
    const aBouncesB =
      configA.canDeflectOthers && configB.canBeDeflected && configB.deflectionBehavior === 'bounce'
    const bBouncesA =
      configB.canDeflectOthers && configA.canBeDeflected && configA.deflectionBehavior === 'bounce'
    ...
    if (projectileA.teamId && projectileB.teamId && projectileA.teamId === projectileB.teamId)
      return
  ...
  private applyClashKnockback(defenderProj: Projectile, attackerProj: Projectile): void {
    ...
    let defender: WorldLife | undefined =
      this.gameState.players.get(defenderProj.ownerId) ||
    ...
    let attacker: WorldLife | undefined =
      this.gameState.players.get(attackerProj.ownerId) ||
    ...
    const impulseMagnitude =
      ...
      config.absorbImpulseMultiplier
    ...
    try {
      ...
      console.warn(
        `⚠️ PROJECTILE: failed to emit BATTLE_DAMAGE_PRODUCED (clash): ${err instanceof Error ? err.message : String(err)}`
      )
    }
  }
