...
export class StatusEffectManager {
  ...
  applyStatusEffect(
    ...
    if (Math.random() > chance) {
      console.log(
        `🛡️ RESIST: ${entity.id} resisted '${type}' (Chance: ${(chance * 100).toFixed(1)}%, Resistance: ${(resistance * 100).toFixed(1)}%)`
      )
      return false
    }
    ...
    if (currentStatus) {
      ...
      console.log(
        `✨ STATUS: ${entity.id} applied '${type}' for ${duration}ms (Val: ${options?.value}, Int: ${options?.interval})`
      )
    }
