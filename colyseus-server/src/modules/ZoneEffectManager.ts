...
export class ZoneEffectManager {
  ...
  createZoneEffect(
    ...
    const zone = new ZoneEffect(id, x, y, ownerId, effects, radius, castTime, duration, tickRate)
    ...
    console.log(
      `🌀 ZONE CREATED: ${id} at ${x},${y} cast=${castTime}ms dur=${duration}ms tick=${tickRate}ms`
    )
    return zone
  }
      ...
      if (!zone.isActive) {
        if (now - zone.createdAt >= zone.castTime) {
          ...
          if (ownerStart && (!ownerStart.isAlive || ownerStart.isStunned)) {
            console.log(
              `🚫 ZONE INTERRUPTED (Final Check): ${id} owner ${ownerStart.id} is stunned/dead`
            )
          ...
          if (ownerCasting && (!ownerCasting.isAlive || ownerCasting.isStunned)) {
            console.log(
              `🚫 ZONE INTERRUPTED (Immediate): ${id} owner ${ownerCasting.id} is stunned/dead`
            )
