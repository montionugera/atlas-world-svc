...
describe('Mob Casting Heading Behavior', () => {
  ...
  test('Mob should (currently) update heading during casting when target moves', () => {
    ...
    // Expected behavior: Mob rotates towards target even while casting, but at heavily reduced speed.
    // Max rotation = limited speed * delta
    // limited speed = rotationSpeed * 0.15
    // delta = 0.016 sec
    ...
    // Note: Mob default rotationSpeed in Mob.ts is Math.PI and casting uses 0.15x speed.
    const expectedHeadingChange = (mob.rotationSpeed * 0.15) * (16 / 1000)
