...
export class BoundaryAwareBehavior extends BaseBehavior {
  ...
  private calculateBoundaryAvoidance(mob: Mob, context: AIContext): { x: number; y: number } {
    ...
    if (distToLeft < this.boundaryThreshold) {
      avoidance.x +=
        (this.avoidanceForce * (this.boundaryThreshold - distToLeft)) / this.boundaryThreshold
    }

    // Avoid right boundary
    if (distToRight < this.boundaryThreshold) {
      avoidance.x -=
        (this.avoidanceForce * (this.boundaryThreshold - distToRight)) / this.boundaryThreshold
    }

    // Avoid top boundary
    if (distToTop < this.boundaryThreshold) {
      avoidance.y +=
        (this.avoidanceForce * (this.boundaryThreshold - distToTop)) / this.boundaryThreshold
    }

    // Avoid bottom boundary
    if (distToBottom < this.boundaryThreshold) {
      avoidance.y -=
        (this.avoidanceForce * (this.boundaryThreshold - distToBottom)) / this.boundaryThreshold
    }
