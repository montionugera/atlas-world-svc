...
export class AIMemoryImpl implements AIMemory {
  ...
  getRecentBehaviors(
    count: number = 5
  ): Array<{ behavior: string; timestamp: number; duration: number }> {
    return this.behaviorHistory.slice(-count)
  }

  clearOldData(maxAge: number = 30000): void {
    // 30 seconds
    const now = Date.now()
    ...
    this.behaviorHistory = this.behaviorHistory.filter(entry => now - entry.timestamp <= maxAge)
  }
}
