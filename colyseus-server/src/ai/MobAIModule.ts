  ...
  private update(): void {
    ...
    try {
      ...
      for (const { mob, config } of this.mobs.values()) {
        const mobStart = performance.now();
        const env: MobEnvironment = this.worldInterface.buildMobEnvironment(mob, config?.perception?.range ?? 50);
      ...
      if (Math.random() < 0.01) { // 1% chance
        ...
        console.log(`⚡ AI Module: ${updateTime.toFixed(2)}ms, ${stats.mobCount} mobs, avg: ${stats.averageUpdateTime.toFixed(2)}ms`);
      }
    } catch (error) {
