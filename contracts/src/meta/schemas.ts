...
export const inventoryDocSchema = z
  .object({
    schemaVersion: z.literal(1),
    stackables: z.array(z.object({ itemId: z.string(), qty: z.number() }).strict()),
    uniques: z.array(z.object({ instanceId: z.string(), itemId: z.string() }).strict()),
  })
...
export const skillsDocSchema = z
  .object({
    schemaVersion: z.literal(1),
    unlocked: z.array(z.object({ skillId: z.string(), level: z.number() }).strict()),
...
const DEFAULT_DOC_FACTORIES: { [K in keyof DefaultDocMap]: () => DefaultDocMap[K] } = {
  [COLLECTIONS.inventory]: () => ({ schemaVersion: 1, stackables: [], uniques: [] }),
...
/**
 * Factory returning a fresh default empty doc for the four non-profile
 * collections. (profile uses DEFAULT_PROFILE.)
 */
export function defaultDoc<K extends keyof DefaultDocMap>(collection: K): DefaultDocMap[K] {
